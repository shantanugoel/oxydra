...
| Telemetry | `tracing` + `tracing-subscriber` | all (from Phase 1) | Phase 1 (basic), Phase 15 (OTel) | Start with `tracing` spans from day one |
| OpenTelemetry | `opentelemetry` + `tracing-opentelemetry` | runtime / gateway | Phase 15 | Distributed traces, metrics, cost attribution |
...
- **Phase 4 defines the `Tool` trait** with `safety_tier()` and `timeout()` — Phase 6 uses these for parallel execution decisions, Phase 11 uses `safety_tier()` for `wasmtime` mount/egress policy, and Phase 16 implements it for MCP.
- **Phase 5 builds the agent loop with `CancellationToken`** — Phase 14 reuses the same token for subagent lifecycle management.
- **`tracing` is introduced in Phase 1** as simple stderr logging — Phase 15 upgrades the subscriber to OpenTelemetry without changing any instrumentation code.
...
- **Phase 12 defines the `Channel` trait** and lands a gateway-backed TUI end-to-end path before external adapters land — Phase 13 extends adapters without changing the trait.
- **Phase 13 introduces deterministic user/channel -> session identity mapping and sender-ID allowlists** before Phase 14 concurrency hardening, preventing multi-channel history fragmentation and unauthorized ingress.
