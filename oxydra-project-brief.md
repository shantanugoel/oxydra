...
- A `chunks_vec` virtual table, utilizing the `sqlite-vec` extension, stores the binary float vectors generated by a local embedding model, enabling rapid nearest-neighbor similarity searches entirely within the database engine.[22]
- A `chunks_fts` virtual table, utilizing SQLite's native FTS5 extension, indexes the raw text to provide exact keyword searches and token matching.[22]

This structure empowers the agent with true hybrid retrieval capabilities. When an agent queries long-term memory, the orchestrator executes cosine similarity search on vectors and BM25 scoring on FTS5 in parallel, then merges results with a weighted ranker.[4, 22] For production readiness, embedded SQLite plus `sqlite-vec` should remain the default path because it is portable and battle-tested. Experimental alternatives such as satoriDB may be evaluated separately for Linux-only high-throughput scenarios, but they should not be the baseline architecture.
...
- **Embedded SQLite DB:** `rusqlite` + `rusqlite_migration` for durable schema evolution.
...
- Instead of a separate vector database server, compile `sqlite-vec` directly into your Rust binary to preserve the zero-overhead philosophy.[22]
...
- Embedded SQLite + `sqlite-vec` remains the default memory architecture for portability and reliability.
...
crates/
  ...
  memory/         # Depends on types. Memory trait + SQLite/sqlite-vec implementation
...
| SQLite | `rusqlite` | memory | Phase 8 | Embedded, battle-tested |
| SQLite migration | `rusqlite_migration` | memory | Phase 8 | Schema evolution without data loss |
| Vector search | `sqlite-vec` (SQLite extension) | memory | Phase 9 | Compile directly into binary |
| Full-text search | SQLite FTS5 | memory | Phase 9 | Built into SQLite |
...
| **Phase 8** | Memory trait + SQLite conversation persistence | `memory` | Phase 1 | Conversations survive process restart; session list/restore works | `Memory` trait: `store`, `recall`, `forget`. SQLite via `rusqlite`. Use `rusqlite_migration` from the start. Store conversations as JSON blobs initially — don't over-normalize. |
| **Phase 9** | Context window management + hybrid retrieval (vector + FTS) | `memory`, `runtime` | Phases 8, 5 | Agent operates within token budget; rolling summarization triggers correctly; hybrid search returns relevant results | `tiktoken-rs` for token counting. Context budget: system + memory + history + tools + buffer. `sqlite-vec` for vectors, FTS5 for keywords. `fastembed-rs` for local embeddings. Address summarizer race condition with epoch counter. |
...
- **Phase 8 uses `rusqlite_migration`** — Phase 9 adds vector/FTS tables via migration, not schema wipe.
